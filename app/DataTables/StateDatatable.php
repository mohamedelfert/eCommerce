...
class StateDatatable extends DataTable
{
    ...
    {
        return $model->newQuery()->with('city_id')->with('country_id')->select('states.*');
    }
    ...
    public function html()
    {
        return $this->builder()
            ...
            ->parameters([
                'buttons' => [
                    ['extend' => 'create', 'className' => 'btn btn-success', 'text' => '<i class="fa fa-plus"> ' . trans('admin.new_state') . ' </i>'],
    ...
    protected function getColumns()
    {
        return [
            ...
            Column::computed('id', 'ID')
                ...
                ->orderable(true)
                ->addClass('text-center'),
            Column::computed('state_name_ar', trans('admin.state_name_ar'))
                ->exportable(true)
                ->searchable(true)
                ->printable(true)
                ->orderable(true)
                ->addClass('text-center'),
            Column::computed('state_name_en', trans('admin.state_name_en'))
                ->exportable(true)
                ->searchable(true)
                ->printable(true)
                ->orderable(true)
                ->addClass('text-center'),
            Column::computed('city_id.city_name_' . session('lang'), trans('admin.city_name_' . session('lang')))
                ->exportable(true)
                ->searchable(true)
                ->printable(true)
                ->orderable(true)
                ->addClass('text-center'),
            Column::computed('country_id.country_name_' . session('lang'), trans('admin.country_name_' . session('lang')))
                ->exportable(true)
                ->searchable(true)
                ->printable(true)
                ->orderable(true)
